...
#REFERENCE _/\_ = > https://github.com/grantjenks/python-sortedcontainers/ 

#USE A MULTISET IN STL
...
def condense(a):
    ...
    a=[str(x) for x in a]
    a="".join(a)
    c=[] #contains consecutive count of ones 
    a=a.split("0")
...
states={} #the array can have only n right shifts, so storing the ans for each right shift! ;)

for s in range(n):
    #perform binary search to find the largest count<=k
    idx=h.bisect_left(k)
    if(idx<l):
        ans=h[idx]
    else:
